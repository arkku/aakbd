/**
 * usbkbd_descriptors.h: USB HID keyboard implementation.
 *
...
#if USB_MAX_KEY_ROLLOVER > (USB_BOOT_PROTOCOL_ROLLOVER - VIRTUAL_KEY_BYTES_IN_REPORT)
// The reserved byte is already occupied by keys
#define RESERVE_BOOT_PROTOCOL_RESERVED_BYTE 0
#else
// We need the reserved byte for padding
...
#if RESERVE_BOOT_PROTOCOL_RESERVED_BYTE || VIRTUAL_KEY_BYTES_IN_REPORT
#define KEY_IN_RESERVED_BYTE 0
#else
#define KEY_IN_RESERVED_BYTE 1
#endif

#define KEYBOARD_REPORT_SIZE        (1 + RESERVE_BOOT_PROTOCOL_RESERVED_BYTE + USB_MAX_KEY_ROLLOVER + VIRTUAL_KEY_BYTES_IN_REPORT)
...
#define CONSUMER_ENDPOINT_NUM       2
#define CONSUMER_ENDPOINT_ADDRESS   (ENDPOINT_DIR_IN | CONSUMER_ENDPOINT_NUM)
#define CONSUMER_ENDPOINT_SIZE      8
#define CONSUMER_INTERFACE_INDEX    (ENABLE_KEYBOARD_ENDPOINT + ENABLE_GENERIC_HID_ENDPOINT)

// Consumer shares EP2. Generic HID OUT moves to EP4.
#if ENABLE_GENERIC_HID_ENDPOINT
#undef GENERIC_HID_ENDPOINT_IN_NUM
#define GENERIC_HID_ENDPOINT_IN_NUM 3
#undef GENERIC_HID_ENDPOINT_OUT_NUM
#define GENERIC_HID_ENDPOINT_OUT_NUM 4
#endif
#endif

#define GENERIC_HID_ENDPOINT_COUNT \
    (ENABLE_GENERIC_HID_ENDPOINT ? 1 + ENABLE_GENERIC_HID_OUTPUT : 0)
#define ENDPOINT_COUNT              (ENABLE_KEYBOARD_ENDPOINT + GENERIC_HID_ENDPOINT_COUNT + MEDIA_KEYS_ENDPOINT)
#define INTERFACES_COUNT            (ENABLE_KEYBOARD_ENDPOINT + ENABLE_GENERIC_HID_ENDPOINT + MEDIA_KEYS_ENDPOINT + ENABLE_DFU_INTERFACE)
...
#endif
#define DFU_INTERFACE_INDEX         (ENABLE_KEYBOARD_ENDPOINT + ENABLE_GENERIC_HID_ENDPOINT + MEDIA_KEYS_ENDPOINT)
...
#endif
#define GENERIC_INTERFACE_INDEX     ENABLE_KEYBOARD_ENDPOINT
...
// Verify all active endpoint numbers are distinct
#if ENABLE_KEYBOARD_ENDPOINT && ENABLE_GENERIC_HID_ENDPOINT
_Static_assert(KEYBOARD_ENDPOINT_NUM != GENERIC_HID_ENDPOINT_IN_NUM,
    "Keyboard and Generic HID IN share endpoint number");
#endif
#if MEDIA_KEYS_ENDPOINT
#if ENABLE_KEYBOARD_ENDPOINT
_Static_assert(KEYBOARD_ENDPOINT_NUM != CONSUMER_ENDPOINT_NUM,
    "Keyboard and Consumer share endpoint number");
#endif
#if ENABLE_GENERIC_HID_ENDPOINT
_Static_assert(GENERIC_HID_ENDPOINT_IN_NUM != CONSUMER_ENDPOINT_NUM,
    "Generic HID IN and Consumer share endpoint number");
#endif
#endif
