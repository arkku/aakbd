...
//
// `WIDE_PAD_ZERO` makes the bottom left corner of the "numpad" section be
// 2U wide, i.e., like the KP 0/Ins key is on standard numpads. Note that
// the active pad will be on the right side (down arrow by default).
//
...
#ifndef WIDE_PAD_ZERO
#define WIDE_PAD_ZERO 0
#endif

#if WIDE_PAD_ZERO && !LAYOUT_ALL_PADS
#define KC_PLEFT KC_NO
#define KC_PDOWN KC_P0
#else
#define KC_PLEFT KC_LEFT
#define KC_PDOWN KC_DOWN
#endif
...
const uint8_t PROGMEM keymaps[1][MATRIX_ROWS][MATRIX_COLS] = {
    [_BASE] = LAYOUT_all(
        ...
        KC_LCTRL, KC_LGUI, KC_LALT,                      KC_SPC,         K_SSPR, KC_RALT, KC_NLCK, KC_RCTRL,     KC_PLEFT, KC_PDOWN, KC_RIGHT
    )
};

#if VIAL_ENABLE

#define LAYOUT_BIT_2U_KP0              0
#define LAYOUT_BIT_ISO_LEFT_SHIFT      1
#define LAYOUT_BIT_SPLIT_SPACE         2
#define LAYOUT_BIT_ENTER_0             3
#define LAYOUT_BIT_ENTER_1             4
#define LAYOUT_BIT_SPLIT_RIGHT_SHIFT_0 5
#define LAYOUT_BIT_SPLIT_RIGHT_SHIFT_1 6
#define LAYOUT_BIT_SPLIT_BACKSPACE     7

const uint16_t vial_default_layout_options PROGMEM = (
    ((WIDE_PAD_ZERO ? 1 : 0) << LAYOUT_BIT_2U_KP0) |
...
const uint8_t vial_keyboard_uid[8] PROGMEM = { 0x01, 0x46, 0x77, 0x41, 0x41, 0x4B, 0x42, 0x44 };

// Vial unlock combo: Esc + Enter (physically far apart, unlikely accidental).
...
// Make sure the unused positions in EEPROM have KC_NO in them, and restore to
// default values if the layout is updated.
void dynamic_keymap_layout_updated(const uint16_t old_opts, const uint16_t opts) {
    ...
    update_keycode(KC_LEFT, 1, 8, !(opts & (1 << LAYOUT_BIT_2U_KP0)));
}
