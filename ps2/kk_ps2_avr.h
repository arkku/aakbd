...
#define ps2_set_pin_state(pin, state) \
    do { \
        if ((state)) { \
...
#define ps2_clk_int_clear_flag() \
    do { \
        EIFR = _BV(PS2_CLK_INT); \
    } while (0)
#define ps2_clk_int_enable() \
    do { \
        EIMSK |= _BV(PS2_CLK_INT); \
    } while (0)
#define ps2_clk_int_disable() \
    do { \
        EIMSK &= ~(_BV(PS2_CLK_INT)); \
    } while (0)
#define ps2_clk_int_on_falling() \
    do { \
        EICRA = (EICRA & ~PS2_CLK_ISC0_BIT) | PS2_CLK_ISC1_BIT; \
    } while (0)
#define ps2_clk_int_on_change() \
    do { \
        EICRA = (EICRA & ~PS2_CLK_ISC1_BIT) | PS2_CLK_ISC0_BIT; \
    } while (0)
...
#define ps2_status_set_output() \
    do { \
        PS2_DDR |= (1U << PS2_STATUS_PIN); \
    } while (0)
#define ps2_status_set_input() \
    do { \
        PS2_DDR &= ~(1U << PS2_STATUS_PIN); \
    } while (0)
#define ps2_status_set(on) \
    do { \
        if (on) { \
...
#define ps2_enable_set_input() \
    do { \
        PS2_DDR &= ~(1U << PS2_ENABLE_PIN); \
    } while (0)
#define ps2_enable_set_pull_up() \
    do { \
        PS2_PORT_REG |= (1U << PS2_ENABLE_PIN); \
    } while (0)
#define ps2_enable_disable_pull_up() \
    do { \
        PS2_PORT_REG &= ~(1U << PS2_ENABLE_PIN); \
    } while (0)
