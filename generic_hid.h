...
#endif
#ifndef GENERIC_HID_ENDPOINT_NUM
#define GENERIC_HID_ENDPOINT_NUM            2
#endif
#ifndef GENERIC_HID_HANDLE_SYNCHRONOUSLY
/// Should the `handle_generic_hid_report` function be called synchronously
/// from the "main thread" rather than from the interrupt handler (the
/// default)? The benefit of this is that it allows delays, etc. in the
/// handler without freezing USB, but the downside is that there can be only
/// one request at a time, and it is not possible to return an error state
/// when handling an invalid request.
#define GENERIC_HID_HANDLE_SYNCHRONOUSLY    0
#endif
...
///
/// See also `GENERIC_HID_HANDLE_SYNCHRONOUSLY`.
